...
def get_args():
    ...
    parser.add_argument('--model-path', type=str, default='2.pt',
...
def main():
    args = get_args()
    args.model_path = '2.pt'

    classes = 10
    model = get_model(
        in_shape=[28, 28], width_hiddens=[64, 64, 64, 64],
    ...
    if args.plot_samples:
        plot_samples(
            ...
            title='examples', path=os.path.join(args.imgs_dir, '2_examples.svg')
        )
    ...
    if args.training:
        ...
        trainer.plot_history(
            title='2_training_history',
            PATH=os.path.join(args.imgs_dir, '2_training_history.svg'),
        )
    ...
    if args.analyzing:
        ...
        analyzer.plot_mistakes(
            ...
            path=os.path.join(args.imgs_dir, '2_MistakenInstances.svg')
        )
        analyzer.plot_confusion_matrix(
            title='Confusion Matrix',
            path=os.path.join(args.imgs_dir, '2_ConfusionMatrix.svg')
        )
