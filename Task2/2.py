...
def get_args():
    parser = argparse.ArgumentParser(description='LeNet - FashionMNIST')
    parser.add_argument('--ExpNo', default=2,
...
def main():
    ...
    model_path = os.path.join(
            args.models_dir,
            '{}_LeNetFashionMNIST.pt'.format(args.ExpNo)
        )
    ...
    if args.training:
        ...
        trainer.plot_history(
            title='Training History of LeNet on FashionMNIST',
            path=os.path.join(
                args.imgs_dir,
                '{}_TrainingHistoryOfLeNetonFashionMNIST.svg'.format(args.ExpNo)
            )
        )
